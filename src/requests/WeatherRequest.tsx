...
export default async function WeatherRequest(props) {
    ...
    try {
        ...
        // const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${props}&appid=${apiKey}&units=metric`);
        ...
        for(let i = 0; i < data.list.length; i++){
            ...
            if(day !== tempDay){
                ...
                maxTemp = data.list[i].main.temp > maxTemp ? data.list[i].main.temp : maxTemp
                minTemp = data.list[i].main.temp < minTemp ? data.list[i].main.temp : minTemp
                temperatureEveryThreeHours.push(data.list[i].main.temp)
            }
        }
        ...
        console.log(perDayVariation["day1"].length)
